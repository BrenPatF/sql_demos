...
SELECT /*+ gather_plan_statistics XPOS */ 
       item_name, bin, item_value, Sum (item_value) OVER (PARTITION BY bin) bin_total
  FROM items_desc
...
SELECT /*+ gather_plan_statistics XPLF */
       item_name, bin, item_value, Sum (item_value) OVER (PARTITION BY bin) bin_value
...
SELECT  /*+ gather_plan_statistics XRSF_D */ 
       r.item_name,
...
SELECT /*+ gather_plan_statistics XMOD */
       item_name, bin, item_value, Max (bin_value) OVER (PARTITION BY bin) bin_value
  FROM (
SELECT * FROM items
  MODEL 
